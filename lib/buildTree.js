const buildTodos = (t) => {
  return t.map('SELECT id, title, created_at, updated_at FROM todos', [], (todo) => {
    return t.any(
...
const buildTodo = (todoId) => {
  const buildTodoTree = (t) => {
    return t.map('SELECT id, title, created_at, updated_at FROM todos  WHERE id = $1', [todoId], (todo) => {
      return t.any(
        'SELECT id, content, complete, created_at, updated_at, todoId FROM todo_items WHERE todoId = $1',
        [todo.id],
      )
